      ...
      <section className="section-pad bg-soft-gradient">
        <div className="container-page">
          ...
          <div className="grid sm:grid-cols-2 gap-6 max-w-3xl mx-auto">
            {DOCTORS.map((d, idx) => (
              <Reveal key={d.name} direction={idx % 2 === 0 ? "left" : "right"} delay={idx * 0.08}>
                <button onClick={() => setOpenDoc(d)} className="group text-left bg-card rounded-3xl overflow-hidden shadow-card border border-border/60 hover:shadow-medium hover:-translate-y-1 transition-smooth w-full">
